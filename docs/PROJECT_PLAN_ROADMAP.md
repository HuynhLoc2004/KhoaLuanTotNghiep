...
| **6** | **Mô hình 3D Khối Thực Thể (.GLB) & Mã QR Di Sản (3D Solid Mesh & QR System)** | Thuật toán Computer Vision phân tích độ sâu chiều không gian Z từ ảnh chụp đơn qua tủ kính, sinh file 3D `.glb` khối đặc có độ dày & đa giác thực sự, mâm xoay 360 trên bục đá cẩm thạch, sinh mã QR in dán bảo tàng, trang du khách quét QR với AI Voice thuyết minh. | ✅ **Hoàn thành** | Đã triển khai & kiểm thử trên nhánh `feature/3d-artifact-qr-viewer` |
