...
| TASK-DOC-QUALITY-001 | Documentation, code/config, secret và data-access Quality Gate trong CI | IN_PROGRESS | `thanh` / `feature/TASK-DOC-QUALITY-001` | ClaimedAt: 2026-08-04T00:10:24+07:00; DESIGN_OPTIONS | TASK-FOUND-001 (DONE), TASK-API-001 (DONE), TASK-DATA-001 (DONE), TASK-CI-001 (DONE) | O/E/P: 3/4.5/6 person-days, LOW-MEDIUM; phụ thuộc lựa chọn scanner và false-positive policy | `scripts/quality/**`, `.github/workflows/quality.yml`, root quality manifests/config (`package.json`, `pnpm-lock.yaml`, `eslint.config.mjs` khi cần), data-access regression chỉ trong `services/api/test/**`, `docs/work/TASK-DOC-QUALITY-001.md`; không sửa runtime product, migration/schema, shared contract/catalog/status trên feature hoặc file `.env` thật | `docs/work/TASK-DOC-QUALITY-001.md`; PLAN_LOCKED quality owners: `docs/04-design/03-ai-experience-quality-gate.md`, `docs/05-quality/04-code-configuration-quality-gate.md`, `docs/05-quality/05-database-query-cache-quality-gate.md` |

Estimate trên chỉ phục vụ chọn việc và phải được rà soát lại khi bắt đầu task.

## Đề xuất hiện tại

`TASK-DOC-QUALITY-001` đang `IN_PROGRESS`, owner `thanh`, branch `feature/TASK-DOC-QUALITY-001`; task đang ở cổng `DESIGN_OPTIONS`, chưa bắt đầu implementation.

- `TASK-DOC-QUALITY-001` đã được claim để mở rộng gate hiện có bằng docs lint, secret/config/dependency scan và regression cho data-access; phải chọn phương án và ghi `PLAN_LOCKED` trước khi code.
