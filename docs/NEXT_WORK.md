...
| TASK-DOC-QUALITY-001 | Documentation, code/config, secret và data-access Quality Gate trong CI | IN_PROGRESS | `thanh` / `feature/TASK-DOC-QUALITY-001` | ClaimedAt: 2026-08-04T00:10:24+07:00; UpdatedAt: 2026-08-04T00:17:59+07:00; `PLAN_LOCKED`: Option C | TASK-FOUND-001 (DONE), TASK-API-001 (DONE), TASK-DATA-001 (DONE), TASK-CI-001 (DONE) | O/E/P: 3/4.5/6 person-days, MEDIUM; hybrid repo-controlled | `scripts/quality/**`, `.github/workflows/quality.yml`, root quality manifests/config (`package.json`, `pnpm-lock.yaml`, `eslint.config.mjs` khi cần), data-access regression chỉ trong `services/api/test/**`, `docs/work/TASK-DOC-QUALITY-001.md`; không sửa runtime product, migration/schema, shared contract/catalog/status trên feature hoặc file `.env` thật | `docs/work/TASK-DOC-QUALITY-001.md`; `DEC-DOC-QUALITY-AUTOMATION-001`; quality owners |

Estimate trên chỉ phục vụ chọn việc và phải được rà soát lại khi bắt đầu task.

## Đề xuất hiện tại

`TASK-DOC-QUALITY-001` đang `IN_PROGRESS`, owner `thanh`, branch `feature/TASK-DOC-QUALITY-001`; Option C hybrid repo-controlled đã `PLAN_LOCKED`.

- `TASK-DOC-QUALITY-001` triển khai markdownlint + Secretlint + project-specific static/regression checks; OSV lockfile scan chạy hosted riêng và không biến network outage thành silent PASS.
