...
| TASK-DOC-QUALITY-001 | Documentation, code/config, secret và data-access Quality Gate trong CI | IN_PROGRESS | `thanh` / `feature/TASK-DOC-QUALITY-001` | ClaimedAt: 2026-08-04T00:10:24+07:00; MergedAt: `e0c4139` / PR `#9`; chưa VERIFIED vì hosted workflow startup failure | TASK-FOUND-001 (DONE), TASK-API-001 (DONE), TASK-DATA-001 (DONE), TASK-CI-001 (DONE), FIX-DOC-QUALITY-CI-001 | O/E/P: 3/4.5/6 person-days, MEDIUM; hybrid repo-controlled | Implementation đã merge; chờ CI fix, exact-commit hosted PASS, user verification và Merge Memory Sync | `docs/work/TASK-DOC-QUALITY-001.md`; `DEC-DOC-QUALITY-AUTOMATION-001`; quality owners |
| FIX-DOC-QUALITY-CI-001 | Sửa GitHub Actions startup failure của hosted OSV scan | IN_PROGRESS | `thanh` / `fix/FIX-DOC-QUALITY-CI-001` | ClaimedAt: 2026-08-04T01:17:20+07:00; `PLAN_LOCKED`: phương án B / PLAN-0029 | TASK-DOC-QUALITY-001 merged tại `e0c4139`; TASK-CI-001 (DONE) | O/E/P: 0.1/0.25/0.5 person-day, HIGH | `.github/workflows/quality.yml`, `docs/work/FIX-DOC-QUALITY-CI-001.md`; không sửa scripts, manifests/lockfiles, runtime, shared coordination files hoặc `.env` thật | `DEC-DOC-QUALITY-OSV-STARTUP-FIX-001`; code/config quality owner |

Estimate trên chỉ phục vụ chọn việc và phải được rà soát lại khi bắt đầu task.

## Đề xuất hiện tại

`FIX-DOC-QUALITY-CI-001` đang `IN_PROGRESS`, owner `thanh`, branch `fix/FIX-DOC-QUALITY-CI-001`; phương án B đã `PLAN_LOCKED`.

- Sửa caller OSV thành normal job dùng direct pinned action và quyền `contents: read`; giữ nguyên ba lockfile và fail theo exit code của scanner.
- `TASK-DOC-QUALITY-001` chỉ chuyển `VERIFIED` sau khi CI trên exact commit PASS và thành viên xác nhận; tiếp đó mới chạy Merge Memory Sync.
- `TASK-SEARCH-001` vẫn `READY` nhưng phải chờ integration turn hiện tại và Merge Memory Sync hoàn tất.
