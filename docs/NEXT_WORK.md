...
| FIX-CLEAN-GATE-001 | Làm root quality gate deterministic trên fresh clone và sửa Admin stale export | IN_PROGRESS | `thanh` / `fix/clean-workspace-quality-gate` | ClaimedAt: 2026-08-03T22:53:36+07:00; PLAN_LOCKED `OPTION-CLEAN-GATE-001/A` | TASK-FOUND-001 (DONE); TASK-ADMIN-001 (DONE); blocks hosted PASS của TASK-CI-001 | O/E/P: 0.1/0.25/0.5 person-day, HIGH | `turbo.json`, `apps/admin/src/index.ts`, `docs/work/FIX-CLEAN-GATE-001.md`; không sửa `packages/ui/**`, `packages/contracts/**`, `.github/workflows/**` hoặc shared status files trên feature | `docs/work/FIX-CLEAN-GATE-001.md` |
...
`TASK-CI-001` do `loc` thực hiện trong `.github/workflows/**`; `FIX-CLEAN-GATE-001` do `thanh` sửa dependency graph/root baseline trong scope riêng, không chạm workflow hoặc UI/contracts. Hosted CI PASS phụ thuộc fix này; `FIX-FORMAT-001` đã merge nhưng giữ `REVIEW` cho đến khi người dùng xác nhận `VERIFIED` và Merge Memory Sync hoàn tất.
