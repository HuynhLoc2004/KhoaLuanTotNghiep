...
`TASK-VOICE-001` đã `DONE` (merge `1bbbe07`, PR `#20`, `thanh` tự test `/voice` trên trình duyệt thật trước merge); `DEC-VOICE-001` khóa Option Web Speech API trình duyệt (SpeechSynthesis/SpeechRecognition) cho MVP, không tích hợp cloud TTS/STT thật vì chưa có credential kiểm chứng được.

- Còn 3 task `READY`: `TASK-RECOGNITION-001` (đề xuất `thanh`, Voice đã xong nên có thể nhận ngay), `TASK-LOCATION-001` và `TASK-CMS-MEDIA-001` (đề xuất `loc`).
- Không còn task `IN_PROGRESS` nào của `thanh` sau khi Voice merge.
