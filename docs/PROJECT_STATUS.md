...
- Fresh-clone quality blocker: hosted run `30829451628` chứng minh lint không resolve workspace types khi declarations trong ignored `dist` chưa được build; warmed local gate còn phát hiện Admin stale re-export. `FIX-CLEAN-GATE-001` (`thanh`) đang xử lý trong `turbo.json` và `apps/admin/src/index.ts`, không nới scope `TASK-CI-001`.
