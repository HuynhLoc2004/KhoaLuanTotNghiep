...
- Keycloak IAM & User History MVP: Keycloak IAM Server Container (`infra/compose.yaml` port 18080), Auth Zod contracts, Express API `/api/v1/auth/*` router (Keycloak JWT verify, profile, bookmarks, history), UI Auth components (AuthHeaderBadge, AuthModal, UserProfileDrawer) & Web profile page đã `VERIFIED`, merge tại `8e76271` qua PR `#12` và hoàn tất Merge Memory Sync.
- Dashboard & Analytics MVP: Dashboard Zod contracts, Express API `/api/v1/dashboard/*` router (summary metrics, 7-day traffic series), UI Dashboard components (MetricCards, TopArtifactsTable, TrafficChart) & Admin Dashboard page đã `VERIFIED`, merge tại `1439807` qua PR `#13` và hoàn tất Merge Memory Sync.
- Mobile AI Tour Guide & Hỏi Đáp Di Sản MVP: AI Guide Zod contracts, Express API `/api/v1/ai/guide/*` router (RAG Hybrid, Museum Citation Gating, TTS Speech), UI Chat Widget components & Web `/ai-guide` page đã `VERIFIED`, merge tại `cae46a2` qua PR `#14` và hoàn tất Merge Memory Sync.
- Mô Phỏng 3D & Xoay Hiện Vật 360° MVP (Digital Twin): 3D Zod contracts, Express API `/api/v1/3d/*` router (scenes, models, A* Graph route calculation), UI 3D Model Viewer & Web `/3d-experience` page đã `VERIFIED`, merge tại `7a4264d` qua PR `#15` và hoàn tất Merge Memory Sync.
- Multilingual Audio Guide & Voice Command MVP: Voice Zod contracts, Express API `/api/v1/voice/*` router (scripts, synthesize, command, admin config/cost-estimate), `DEC-VOICE-001` Web Speech API trình duyệt engine, UI Voice Player Widget & Web `/voice` page đã `VERIFIED` (thanh tự test trình duyệt thật), merge tại `1bbbe07` qua PR `#20` và hoàn tất Merge Memory Sync.
...
- Feature/task: `TASK-FOUND-001` (`thanh`) `DONE`, merge tại `3d8b971`; `TASK-INFRA-001` (`loc`) `DONE`, merge tại `847251c`; `TASK-WEB-001` (`thanh`) `DONE`, merge tại `03dfcd4`; `TASK-API-001` (`thanh`) `DONE`, merge tại `8afa7c8`; `TASK-ADMIN-001` (`thanh`) `DONE`, merge tại `5ce59f2`; `TASK-DATA-001` (`thanh`) `DONE`, merge tại `d75cdf8`; `TASK-TIMELINE-001` (`thanh`) `DONE`, merge tại `92c7caa`; `TASK-SEARCH-001` (`loc`) `DONE`, merge tại `400d328`; `TASK-AUTH-001` (`loc`) `DONE`, merge tại `8e76271`; `TASK-DASHBOARD-001` (`loc`) `DONE`, merge tại `1439807`; `TASK-AI-GUIDE-001` (`loc`) `DONE`, merge tại `cae46a2`; `TASK-3D-NAV-001` (`loc`) `DONE`, merge tại `7a4264d`; `FIX-LINT-001` (`thanh`) `DONE`, merge tại `cc1c600`; `FIX-FORMAT-001` (`thanh`) `DONE`, merge tại `8d199db`; `TASK-CI-001` (`loc`) `DONE`, merge tại `be2a18e`; `FIX-CLEAN-GATE-001` (`thanh`) `DONE`, merge tại `8bf9c9e`; `TASK-VOICE-001` (`thanh`) `DONE`, merge tại `1bbbe07`.
