...
85. After team confirmation, Codex owns the Markdown-only coordination Git sequence: verify branch/status/remote, isolate only accepted `.md` planning files, update local `develop` safely (prefer a separate coordination worktree so the active feature branch stays untouched), commit and push to `origin/develop`, verify the remote commit, then confirm the active task branch is unchanged. Do not ask the user to remember this sequence. Stop on non-Markdown files, dirty unrelated files, divergence, conflict, rejected push, missing remote/tracking or ambiguous ownership; never force-push, discard changes, or bring feature code/commits into `develop`.
86. Every accepted shared plan/task coordination update must update `README.md` in the same `develop` commit. Keep README as a concise current-status/router view (Plan Snapshot revision, active task owners/branches and sync rule) and link to owner documents rather than duplicating detailed plans. After push, verify README is present in the remote `develop` commit before returning to the feature branch.
87. When implementation is ready for handoff, Codex may report tests and propose exact feature-branch push/review/merge commands, but the user must execute the feature push and merge decision. Codex must not automatically push the feature branch or merge it into `develop`, even when documentation-only coordination automation is enabled.
88. Enforce scope isolation without requiring collaborators to monitor, message, remind or inspect each other's feature branches. At session start, Codex must fetch/read the latest remote `develop` coordination state, identify the confirmed owner/write scope, and permit edits only inside that scope plus explicitly owned documentation.
89. Unknown work on another feature branch is not a blocker when published write scopes do not overlap. If the requested change would touch another owner's scope or an unassigned shared file/contract, stop before editing and publish a Markdown coordination request on `develop`; do not inspect the other branch merely to infer what it contains.
90. Feature branches must not modify shared coordination/status files: `README.md`, `CURRENT_TASK.md`, `docs/NEXT_WORK.md`, `docs/PLAN_SNAPSHOT.md`, `docs/PROJECT_STATUS.md`, `docs/IMPLEMENTATION_INDEX.md`, `docs/04-design/02-ui-component-registry.md`, shared contract/integration catalogs, or another task's report. Codex updates these only through the Markdown-only `develop` coordination worktree.
91. Each implementation branch records all task-local plan, session ledger, decisions, evidence, tests and handoff in `docs/work/<TASK-ID>.md` created from `docs/templates/task-work-report-template.md`. A feature PR may change its own task report and owned code/spec only. Shared owner/index/status promotion happens after merge through Merge Memory Sync on `develop`.
92. Before opening/updating a feature PR, Codex must compare changed paths against the shared-file denylist and the task write scope. If forbidden shared files are present, move durable task evidence into the task report and restore shared files to the accepted `origin/develop` version before asking the user to push.
93. Enforce a single-writer integration turn for `develop`: only one member may merge/push implementation into `develop` at a time. The next member waits until the prior merge and its Markdown Merge Memory Sync are both verified on remote `develop`.
94. Before a user-owned manual feature merge, verify a clean tree, `VERIFIED` task status, shared-file denylist, current integration-turn ownership and latest remote `develop`. Recommend `git pull --ff-only origin develop` while on local `develop`; stop on divergence, conflict, rejected push or unexpected remote movement.
95. Do not require merging `develop` into a completed feature merely as conflict prevention. With non-overlapping scopes, merge the verified feature once into the latest local `develop`; bring `develop` into a feature only when development/testing must continue against new shared code or an actual conflict requires branch-local resolution.
96. After a successful implementation push to `develop`, keep the integration turn closed until Codex completes and verifies the required Markdown-only Merge Memory Sync on remote `develop`; only then announce that the next owner may integrate.
97. After Codex has presented the selected task, confirmed identity/owner, branch, dependencies, write scope and required plan, an explicit user confirmation to start authorizes Codex to execute the complete safe task-startup sequence on the current workspace without asking the user to run each Git command.
98. The AI-managed startup sequence is: publish/verify the Markdown coordination claim on remote `develop`; verify clean branch/tracking/worktree; update local `develop` with fast-forward-only; create the new feature branch or switch/pull the existing owned branch; open the task report/contribution session; record `PRE_CODE_PLAN_SYNC: PASS`; then begin scoped implementation.
99. Stop AI-managed startup on dirty unrelated changes, divergence, detached HEAD, conflict, missing/rejected remote, ambiguous ownership/scope or branch mismatch. Startup authorization never includes rebase, feature commit/push, PR creation/merge, implementation merge into `develop` or force-push.
100. Preventive Quality Rules for Implementation & Task Reports:
    a. Active Task Reports (`docs/work/<TASK-ID>.md`): Must contain all 11 required headings: `## Identity and Git context`, `## Objective and write scope`, `## PLAN_LOCKED`, `## System sequence`, `## Architecture and State Flow`, `## Technology inventory`, `## Acceptance criteria`, `## Feature lifecycle and Contribution ledger`, `## Testing evidence`, `## Handoff`, `## Change history`.
    b. Environment Variables: Every `process.env.KEY_NAME` accessed in runtime code (e.g. `services/api/src/`) MUST be declared in `infra/.env.example` to pass `[CONFIG_ENV_COVERAGE]`.
    c. TypeScript Template Expressions: Numbers in template literals MUST be explicitly converted with `String(val)` or `val.toLocaleString()` to comply with `@typescript-eslint/restrict-template-expressions`.
    d. Async Test Server Hooks: HTTP test servers started in `before` hooks MUST be wrapped in `await new Promise<void>((resolve) => app.listen(0, resolve))` to prevent empty `baseUrl` failures in `node:test`.
    e. Code Formatting: Always execute `npx prettier --write .` before committing implementation changes.
