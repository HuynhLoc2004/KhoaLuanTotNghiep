...
export async function enqueue3DReconstruction(
  ...
  // 1. Kiểm tra cache dựa trên SHA256 (kèm mã phân biệt mặt sau độc lập v3 đồng bộ màu)
  const fileHash = computeFileHash(imagePath) + (backImagePath ? `_back_${computeFileHash(backImagePath)}` : '_dorsal_synced_v3');
