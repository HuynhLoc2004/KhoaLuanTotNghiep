...
export function renderHeader(): string {
  return `
    <header id="main-header" class="glass-nav sticky top-0 z-50 px-6 py-4 transition-all">
      <div class="max-w-7xl mx-auto flex justify-between items-center">
        <a id="nav-brand" href="/" class="flex items-center gap-3 group text-slate-100 hover:text-amber-400 transition-colors">
          <div class="w-10 h-10 rounded-full bg-gradient-to-br from-amber-500 to-red-800 p-[1px] shadow-lg shadow-amber-500/20 group-hover:scale-105 transition-transform">
          ...
          <span class="font-heading font-extrabold text-lg tracking-wider text-slate-100 group-hover:text-amber-300">BẢO TÀNG LỊCH SỬ TP.HCM</span>
        </a>
        <nav id="main-nav" class="hidden md:flex gap-6 items-center font-medium text-sm">
